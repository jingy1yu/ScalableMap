...
# def allocate(ratio, total_num):
#     t_ratio = ratio.copy()
#     l = np.zeros_like(ratio)
...
def allocate_new2(ratio, total_num):
    ...
    left_num = min(total_num - int(l.sum()), 25)
...
class LiDARInstanceLines(object):
    ...
    def bbox(self):
        ...
        instance_bbox_tensor[:, 1::2] = torch.clamp(instance_bbox_tensor[:, 1::2], min=-self.max_y, max=self.max_y)
    ...
    def shift_4stage_fixed_num_control_points_index(self):
        ...
        for instance in self.instance_list:
            ...
            is_poly = is_poly.all()
            if pts_num > 2:  # 只保留特征明显的拐点
                poly_pts = simplify_coords(poly_pts, 0.05)  # TODO 合併閾值定爲0.05
                pts_num = poly_pts.shape[0]
            if pts_num == 2 or pts_num > self.fixed_num:  # 如果只有两个点或超过25个点，直接按照距离插值25个
                distances = np.linspace(0, instance.length, self.fixed_num)
                ...
                if pts_num == self.fixed_num:
                    ...
                    allo_pts_num_per_area = allocate_new2(ratio, self.fixed_num - pts_num)  # 根据比例，将真值辅助点分配到各个区域
                    # 根据点数均匀插值
                    distances = []
                    dis_sum = 0
                    control_index = 0
    ...
    # @property
    # def shift_5stage_fixed_num_sampled_points(self):
    #     """
    ...
    #         instances_tensor = torch.stack(instances_list, dim=0)
    #         instances_tensor = instances_tensor.to(dtype=torch.float32)
    #         outer_instance.append(instances_tensor)
    #         fixed_num = 2 * fixed_num - 1
...
class SMPNuScenesLocalMapDataset(CustomNuScenesDataset):
