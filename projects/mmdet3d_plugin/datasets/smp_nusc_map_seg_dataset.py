...
def allocate_new2(ratio, total_num):
    """ allocate auxiliary points between control points """
    t_ratio = ratio.copy()
    l = np.floor(np.multiply(total_num, ratio)).astype(np.int64)
    left_num = min(total_num - int(l.sum()), 17)
    for i in range(0, left_num):
        c = np.argmax(ratio, axis=0)
        l[c] = l[c] + 1
...
class LiDARInstanceLines(object):
    ...
    def __init__(self,
        ...
        self.max_y = pc_range[4]
        self.min_y = pc_range[1]
    ...
    def bbox(self):
        ...
        instance_bbox_tensor[:, 1::2] = torch.clamp(instance_bbox_tensor[:, 1::2], min=self.min_y, max=self.max_y)
        return instance_bbox_tensor

    @property
    def shift_4stage_fixed_num_control_points_index(self):
        """interpolate the original point set to hierarchical sparse map representation (HSMR)"""
        assert len(self.instance_list) != 0
        instances_list = []
        index_list = []
        ...
        for instance in self.instance_list:
            shift_pts_list = []
            temp_index = np.zeros(self.fixed_num)
            temp_index[0] = temp_index[-1] = 1
            shift_temp_index = []
            poly_pts = np.array(list(instance.coords))
            pts_num = poly_pts.shape[0]
            start_pts = poly_pts[0]
            end_pts = poly_pts[-1]
            is_poly = np.equal(start_pts, end_pts).all()
            # simplify
            if pts_num > 2:
                poly_pts = simplify_coords(poly_pts, 0.05)  # threshold
                pts_num = poly_pts.shape[0]
            if pts_num == 2 or pts_num > self.fixed_num:
                distances = np.linspace(0, instance.length, self.fixed_num)
                sampled_points = np.array(
                    [list(instance.interpolate(distance).coords) for distance in distances]).reshape(-1, 2)
                if pts_num > 2:
                    temp_index = np.ones(self.fixed_num)
                if is_poly:
                    pts_to_shift = sampled_points[:-1, :]
                    index_to_shift = temp_index[:-1]
                    for shift_right_i in range(final_shift_num):
                        shift_pts = np.roll(pts_to_shift, self.fixed_num - shift_right_i - 1, axis=0)
                        t_index = np.roll(index_to_shift, self.fixed_num - shift_right_i - 1, axis=0)
                        pts_to_concat = shift_pts[0]
                        pts_to_concat = np.expand_dims(pts_to_concat, axis=0)
                        t_index[0] = 1
                        index_to_concat = t_index[0]
                        index_to_concat = np.expand_dims(index_to_concat, axis=0)
                        shift_pts = np.concatenate((shift_pts, pts_to_concat), axis=0)
                        shift_pts_list.append(shift_pts)
                        t_index = np.concatenate((t_index, index_to_concat), axis=0)
                        shift_temp_index.append(t_index)
                else:
                    flip_sampled_points = np.flip(sampled_points, axis=0)
                    shift_pts_list.append(sampled_points)
                    shift_pts_list.append(flip_sampled_points)
                    shift_temp_index.append(temp_index)
                    shift_temp_index.append(temp_index)
            else:  # allocate auxiliary points after simplifying
                if pts_num == self.fixed_num:
                    sampled_points = poly_pts
                    temp_index = np.ones(self.fixed_num)
                else:
                    distance = np.linalg.norm((poly_pts[1:, :] - poly_pts[:-1, :]), axis=-1, ord=2)
                    ratio = np.divide(distance, instance.length+1e-5)
                    # allocate auxiliary points according to the ratio
                    allo_pts_num_per_area = allocate_new2(ratio, self.fixed_num - pts_num)
                    # fix number interpolation
                    distances = []
                    dis_sum = 0
                    control_index = 0
                    for ind, area in enumerate(allo_pts_num_per_area):
                        if area > 0:
                            distances.append(np.linspace(dis_sum, distance[ind] + dis_sum, int(area + 2))[:-1])
                        else:
                            distances.append(np.array(dis_sum).reshape(-1))
                        dis_sum += distance[ind]
                        control_index += int(area + 1)
                        temp_index[control_index] = 1
                    distances.append(np.array(instance.length).reshape(-1))
                    distances = np.concatenate(distances, axis=0)
                    sampled_points = np.array(
                        [list(instance.interpolate(distance).coords) for distance in distances]).reshape(-1, 2)
                pts_num, coords_num = sampled_points.shape
                shift_num = pts_num - 1
                if is_poly:
                    pts_to_shift = sampled_points[:-1, :]
                    index_to_shift = temp_index[:-1]
                    ...
                    shift_pts_list.append(flip_sampled_points)
                    shift_temp_index.append(temp_index)
                    shift_temp_index.append(temp_index[::-1])

            multi_shifts_pts = np.stack(shift_pts_list, axis=0)
            shifts_num, pts_num, _ = multi_shifts_pts.shape
            multi_index = np.stack(shift_temp_index, axis=0)

            multi_shifts_pts_tensor = to_tensor(multi_shifts_pts)
            multi_index_tensor = to_tensor(multi_index)
            ...
            multi_shifts_pts_tensor[:, :, 0] = torch.clamp(multi_shifts_pts_tensor[:, :, 0], min=-self.max_x,
                                                           max=self.max_x)
            multi_shifts_pts_tensor[:, :, 1] = torch.clamp(multi_shifts_pts_tensor[:, :, 1], min=self.min_y,
                                                           max=self.max_y)
            if multi_shifts_pts_tensor.shape[0] < final_shift_num:
                padding = torch.full([final_shift_num - multi_shifts_pts_tensor.shape[0], pts_num, 2],
                                     self.padding_value)
                multi_shifts_pts_tensor = torch.cat([multi_shifts_pts_tensor, padding], dim=0)
            instances_list.append(multi_shifts_pts_tensor)
            index_list.append(multi_index_tensor)
        instances_tensor = torch.stack(instances_list, dim=0)
        instances_tensor = instances_tensor.to(dtype=torch.float32)
        return instances_tensor, index_list
...
class VectorizedLocalMap(object):
    ...
    def gen_vectorized_samples(self, location, lidar2global_translation, lidar2global_rotation):
        ...
        if self.pc_range[4] + self.pc_range[1] != 0:
            patch_box = (map_pose[0], map_pose[1], self.pc_range[4] * 2, self.patch_size[1])
        else:
        ...
        gt_instance = LiDARInstanceLines(gt_instance, self.sample_dist,
                                         ...
                                         patch_size=self.patch_size, pc_range=self.pc_range)

        lane_mask = self.nusc_maps[location].get_map_mask(patch_box, patch_angle, ['drivable_area'], (200, 100))

        anns_results = dict(
            gt_vecs_pts_loc=gt_instance,
            gt_vecs_label=gt_labels,
            gt_drivable=lane_mask[0]
        )
        return anns_results
    ...
    def ped_poly_geoms_to_instances(self, ped_geom):
        ...
        local_patch = box(-max_x - 0.2, min_y - 0.2, max_x + 0.2, max_y + 0.2)
        exteriors = []
        interiors = []
...
class SMPNuScenesSegLocalMapDataset(CustomNuScenesDataset):
    r"""NuScenes Dataset add static map elements
    """
    def __init__(self,
                 map_ann_file=None,
                 queue_length=4,
        ...
        self.vector_map = VectorizedLocalMap(kwargs['data_root'],
                                             ...
                                             padding_value=self.padding_value, pc_range=pc_range)
    ...
    def vectormap_pipeline(self, example, input_dict):
        ...
        gt_drivable = to_tensor(anns_results['gt_drivable']).to(torch.int64)
        ...
        example['gt_drivable'] = DC(gt_drivable, cpu_only=True)
        return example
